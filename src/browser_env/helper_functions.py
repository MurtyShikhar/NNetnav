...
class RenderHelper(object):
    ...
    def __init__(self, config: dict, result_dir: str, action_set_tag: str) -> None:
        _config_str = ""
    ...
    def render(
        ...
        if "url" in observation:
            new_content += f"<h3 class='url'><a href={observation['url']}>URL: {observation['url']}</a></h3>\n"
        else:
        ...
        if meta_data["env_type"] == "webarena":
            ...
            # text observation is raw html which we want to display as a string instead of rendering it
            raw_html_observation = observation["text"]
            # wrap the raw html in a pre tag to display it as a string
            # replace < with &lt; and > with &gt; & with &amp; to prevent html rendering
            raw_html_observation = raw_html_observation.replace("&", "&amp;")
            raw_html_observation = raw_html_observation.replace("<", "&lt;")
            raw_html_observation = raw_html_observation.replace(">", "&gt;")
            new_content += (
                f"<div class='state_obv'><pre>{raw_html_observation}</pre><div>\n"
            )
        ...
        # action
        if meta_data["env_type"] != "webarena":
            raw_pred = action["raw_prediction"]
            parsed_pred = action["parsed_response"]
            action_str = f"<div class='raw_parsed_prediction' style='background-color:grey'><pre>{raw_pred}</pre></div>"
            action_str += f"<div class='parsed_action' style='background-color:yellow'><pre>{parsed_pred}</pre></div>"
        else:
