...
@dataclass
class TogetherAIModelArgs(BaseModelArgs):
    """Serializable object for instantiating a generic chat model with an OpenAI
    model, using TogetherAI."""

    def make_model(self):
        return TogetherAIChatModel(
            model_name=self.model_name,
            temperature=self.temperature,
            max_tokens=self.max_new_tokens,
        )
...
class VLLMChatModel(ChatModel):
    def __init__(
        ...
        super().__init__(
            ...
            client_class=OpenAI,
            client_args={"base_url": "https://g4ff95j6-8000.usw3.devtunnels.ms/v1"},
            pricing_func=None,
        )
...
class TogetherAIChatModel(ChatModel):
    def __init__(
        self,
        model_name,
        api_key=None,
